...
// [[Rcpp::export]]
arma::mat gp_mcla(arma::mat covmat,
                  arma::vec targets,
                  int n_classes,
                  double tol = 1e-10,
                  int max_iters = 20){
    ...
    while(arma::mean(arma::pow(f_old - f, 2)) > tol){
        ...
        for(int c = 0; c < n_classes; c++){
            ...
            E.submat(start_loc, start_loc, end_loc, end_loc) = Dc * arma::solve(arma::trimatu(Lc.t()), arma::solve(arma::trimatl(Lc), Dc));
        ...
        a_vec = b_vec - c_vec + E * R * arma::solve(arma::trimatu(M.t()), arma::solve(arma::trimatl(M), R.t() * c_vec));
